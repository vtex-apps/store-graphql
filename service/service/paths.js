...
const paths = {
    ...
    products: (account, { query, category, brands, collection, pageSize, availableOnly, sort }) => `http://api.vtex.com/${account}/products/?query=${encodeURIComponent(query)}&category=${category}&brands=${brands}&collection=${collection}&pageSize=${pageSize}&availableOnly=${availableOnly}&sort=${sort}`,
    ...
    shipping: (account, { skuId, postalCode }) => `http://${account}.vtexcommercestable.com.br/api/checkout/pub/orderForms/simulation?request.items[0].id=${skuId}&request.items[0].quantity=1&request.items[0].seller=1&request.postalCode=${postalCode}&request.country=BRA`,
    ...
    search: (account, { query }) => `http://${account}.vtexcommercestable.com.br/api/catalog_system/pub/products/search/${encodeURIComponent(query)}`,
};
