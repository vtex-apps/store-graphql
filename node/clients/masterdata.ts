 ...
 * to a JSON format.
 */
const parseFieldsToJson = (fields: KeyValue[]) => mergeAll(
...
export class MasterData extends JanusClient {
  ...
  public getDocument = (acronym: string, id: string, fields: string[]) => this.get<Document>(
    this.routes.document(acronym, id),
    { metric: 'masterdata-getDocument', params: { _fields: fields } }
  )

  public createDocument = (acronym: string, fields: KeyValue[]) => this.post<Document>(
    this.routes.documents(acronym),
    { metric: 'masterdata-createDocument', params: parseFieldsToJson(fields) }
  )

  public updateDocument = (acronym: string, id: string, fields: KeyValue[]) => this.patch<Document>(
    this.routes.document(acronym, id),
    { metric: 'masterdata-updateDocument', params: parseFieldsToJson(fields) }
  )

  public searchDocuments = (acronym: string, fields: string[], where: string, pagination: PaginationArgs) => this.get<any[]>(
    this.routes.search(acronym),
    { headers: paginationArgsToHeaders(pagination), metric: 'masterdata-searchDocuments', params: { _fields: fields, _where: where } }
  )

  public deleteDocument = (acronym: string, id: string) => this.delete(
    this.routes.document(acronym, id),
    { metric: 'masterdata-deleteDocument' }
  )

  public uploadAttachment = (acronym: string, id: string, fields: string, formData: FormData) => this.post<any>(
    this.routes.attachments(acronym, id, fields),
    formData,
    { headers: formData.getHeaders(), metric: 'masterdata-uploadAttachment' }
  )
  ...
  private get routes() {
    ...
    return {
      attachments: (acronym: string, id: string, fields: string) => `${acronym}/documents/${id}/${fields}/attachments`,
