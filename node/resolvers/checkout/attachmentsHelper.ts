...
const isInitialItemMissing = (parentCartItem: OrderFormItem, orderForm: any) =>
  ...
  const metadataItems = path(['itemMetadata', 'items'], orderForm) as MetadataItem[]
  const metadataItem = find(propEq('id', initialItem.id), metadataItems) as MetadataItem
  return {
    initialQuantity: initialItem.initialQuantity,
    name: prop('name', metadataItem),
