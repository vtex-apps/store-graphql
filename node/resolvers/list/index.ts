...
import { acronymList, acronymListProduct, fields, fieldsListProduct, validateItems, Item } from './util'

const getListItemsWithProductInfo = (items: Item[], catalog: CatalogDataSource) => Promise.all(
  map(async (item: Item) => {
...
const getListItems = async (itemsId: string[], catalog: CatalogDataSource, masterdata: MasterData) => {
  const items: Item[] = itemsId ? await Promise.all(map((id: string) =>
    masterdata.getDocument(acronymListProduct, id, fieldsListProduct), itemsId)) as Item[] : []
...
const deleteItems = (items: Item[], masterdata: MasterData) => (
  items && Promise.all(items.map((item: Item) => masterdata.deleteDocument(acronymListProduct, path(['id'], item) as string)))
)
